...
class GsCfg {
  constructor () {
    /** 默认设置 */
    this.defSetPath = './plugins/call_of_seven_saints/defSet/'
    this.defSet = {}

    /** 用户设置 */
    this.configPath = './plugins/call_of_seven_saints/config/'
    this.config = {}
  ...
  cpCfg (app, name) {
    if (!fs.existsSync('./plugins/call_of_seven_saints/config')) {
      fs.mkdirSync('./plugins/call_of_seven_saints/config')
    }

    let set = `./plugins/call_of_seven_saints/config/${app}.${name}.yaml`
    if (!fs.existsSync(set)) {
      fs.copyFileSync(`./plugins/call_of_seven_saints/defSet/${app}/${name}.yaml`, set)
    }
  }
